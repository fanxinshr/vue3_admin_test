...
export const useCategoryStore = defineStore("Category", {
    ...
    actions: {
        ...
        async getCategory2List(category1Id: number) {
            const result: CategoryResponseData = await reqC2Category(this.category1Id)
            if (result.code == 200) {
                // 存储二级分类的数据
        ...
        async getCategory3List(category2Id: number) {
            const result: CategoryResponseData = await reqC3Category(this.category2Id)    
            if (result.code == 200) {
                // 存储三级分类的数据
    ...
    getters: {
        ...
        getCategory1NameById(state){
            return (id:number|string)=>{
                const result = state.category1List.find(item=>item.id == id)
                return result?.name
            }
        },
        // 获取二级分类的名称
        getCategory2NameById(state){
            return (id:number|string)=>{
                const result = state.category2List.find(item=>item.id == id)
                return result?.name
            }
        },
        // 获取三级分类的名称
